...
namespace Lerawin.Utilities
{
    public class Tools
    {
        ...
        public static Entity GetFovPlayerHead(int FOV)
        {
            Entity e = null;
            foreach (Entity Player in G.EntityList)
            {
                if (Player.Valid)
                {
                    ...
                    {
                        // Calculate FOV from Recoil Position, on middle of screen.
                        ...
                        Vector2 RecoilScreen;
                        RecoilScreen.X = mx;
                        RecoilScreen.Y = my;

                        float dist = Vector2.Distance(RecoilScreen, vector);
                        if (dist < FOV && e == null)
                        {
                            e = Player;
                        }
                        else if (dist < FOV)
                        {
                            ...
                            if (Vector2.Distance(RecoilScreen, vector4) > dist)
                            {
                                e = Player;
                            }
                        }
        ...
        public static Entity GetFovPlayerNeck(int FOV)
        {
            Entity e = null;
            foreach (Entity Player in G.EntityList)
            {
                if (Player.Valid)
                {
                    Vector2 vector = WorldToScreen(Player.NeckPosition);
                    if (!IsNullVector2(vector))
                    {
                        float dy = Main.ScreenSize.Height / 90;
                        float dx = Main.ScreenSize.Width / 90;
                        ...
                        {
                            Vector2 vector3 = WorldToScreen(e.NeckPosition);
                            Vector2 vector4;
                            vector4 = new Vector2(vector3.X, vector3.Y);
                            if (Vector2.Distance(RecoilScreen, vector4) > dist)
                            {
        ...
        public static Entity GetFovPlayerChest(int FOV)
        {
            Entity e = null;
            foreach (Entity Player in G.EntityList)
            {
                if (Player.Valid)
                {
                    Vector2 vector = WorldToScreen(Player.ChestPosition);
                    if (!IsNullVector2(vector))
                    {
                        float dy = Main.ScreenSize.Height / 90;
                        float dx = Main.ScreenSize.Width / 90;
                        ...
                        {
                            Vector2 vector3 = WorldToScreen(e.ChestPosition);
                            Vector2 vector4;
                            vector4 = new Vector2(vector3.X, vector3.Y);
                            if (Vector2.Distance(RecoilScreen, vector4) > dist)
                            {
        ...
        public static Entity GetFovPlayerLowerChest(int FOV)
        {
            Entity e = null;
            foreach (Entity Player in G.EntityList)
            {
                if (Player.Valid)
                {
                    Vector2 vector = WorldToScreen(Player.LowerChestPosition);
                    if (!IsNullVector2(vector))
                    {
                        float dy = Main.ScreenSize.Height / 90;
                        float dx = Main.ScreenSize.Width / 90;
                        ...
                        {
                            Vector2 vector3 = WorldToScreen(e.LowerChestPosition);
                            Vector2 vector4;
                            vector4 = new Vector2(vector3.X, vector3.Y);
                            if (Vector2.Distance(RecoilScreen, vector4) > dist)
                            {
        ...
        public static Entity GetFovPlayerStomach(int FOV)
        {
            Entity e = null;
            foreach (Entity Player in G.EntityList)
            {
                if (Player.Valid)
                {
                    Vector2 vector = WorldToScreen(Player.StomachPosition);
                    if (!IsNullVector2(vector))
                    {
                        float dy = Main.ScreenSize.Height / 90;
                        float dx = Main.ScreenSize.Width / 90;
                        ...
                        {
                            Vector2 vector3 = WorldToScreen(e.StomachPosition);
                            Vector2 vector4;
                            vector4 = new Vector2(vector3.X, vector3.Y);
                            if (Vector2.Distance(RecoilScreen, vector4) > dist)
                            {
        ...
        public static Entity GetFovPlayerLegs(int FOV)
        {
            Entity e = null;
            foreach (Entity Player in G.EntityList)
            {
                if (Player.Valid)
                {
                    Vector2 vector = WorldToScreen(Player.LegsPosition);
                    if (!IsNullVector2(vector))
                    {
                        float dy = Main.ScreenSize.Height / 90;
                        float dx = Main.ScreenSize.Width / 90;
                        ...
                        {
                            Vector2 vector3 = WorldToScreen(e.LegsPosition);
                            Vector2 vector4;
                            vector4 = new Vector2(vector3.X, vector3.Y);
                            if (Vector2.Distance(RecoilScreen, vector4) > dist)
                            {
